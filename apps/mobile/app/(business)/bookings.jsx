  ...
  return (
    <View style={{ flex: 1, backgroundColor: COLORS.bg }}>
      <View style={{ paddingHorizontal: 20, paddingTop: 60, paddingBottom: 16 }}>
        <Text style={{ color: COLORS.brand, fontSize: 11, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 4 }}>Gestione</Text>
        <Text style={{ fontFamily: FONT_FAMILY.displayHeavy, color: '#fff', fontSize: 24, marginBottom: 4 }}>Prenotazioni</Text>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 100 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={COLORS.brand} />}>
        {filtered.length === 0 ? (
          <View style={{ alignItems: 'center', paddingVertical: 48 }}>
            <Text style={{ color: COLORS.textMuted, fontSize: 14 }}>Nessuna prenotazione.</Text>
          </View>
        ) : filtered.map(b => (
          <View key={b.id} style={{ backgroundColor: COLORS.bgElev2, borderRadius: 14, padding: 16, marginBottom: 10, borderWidth: 1, borderColor: b.checked_in ? 'rgba(74,222,128,0.2)' : COLORS.borderSubtle }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <View style={{ flex: 1, marginRight: 12 }}>
                ...
                <Text style={{ color: COLORS.textMuted, fontSize: 12, marginBottom: 2 }}>{b.profiles?.phone || 'Nessun telefono'}</Text>
                ...
                {checkingIn === b.id
                  ...
                  : b.checked_in
                    ? <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                        ...
                        <Text style={{ color: COLORS.success, fontWeight: '700', fontSize: 12 }}>Entrato</Text>
                      </View>
                    : <Text style={{ color: '#fff', fontWeight: '700', fontSize: 12 }}>Check-in</Text>
                }
              </Pressable>
            </View>
          </View>
