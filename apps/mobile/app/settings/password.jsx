...
export default function ChangePasswordScreen() {
  ...
  async function handleSave() {
    ...
    if (newPassword !== confirm) {
      setError(t('settings.pwErrorMismatch'));
      return;
    }
    setLoading(true);
    ...
    if (err) {
      setError(err.message.includes('same password')
        ? t('settings.pwErrorSame')
        : t('settings.pwErrorGeneric'));
      return;
    }
    setSuccess(true);
    setNewPassword('');
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: COLORS.bg }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <View style={{ paddingHorizontal: 20, paddingTop: 60, paddingBottom: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <Pressable onPress={handleSave} disabled={loading} hitSlop={6} style={({ pressed }) => ({ opacity: loading || pressed ? 0.5 : 1 })}>
          {loading ? <ActivityIndicator color={COLORS.textPrimary} size="small" /> : <Text style={{ fontFamily: FONT_FAMILY.display, color: COLORS.textPrimary, fontSize: 16 }}>{t('common.save')}</Text>}
        </Pressable>
      </View>
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 48 }} keyboardShouldPersistTaps="handled">
        <Text style={{ color: COLORS.brand, fontSize: 11, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 4 }}>{t('settings.pwSectionEyebrow')}</Text>
        <Text style={{ fontFamily: FONT_FAMILY.displayHeavy, color: COLORS.textPrimary, fontSize: 24, marginBottom: 8 }}>{t('settings.changePassword')}</Text>
        ...
        {success ? (
          <View style={{ backgroundColor: 'rgba(74,222,128,0.1)', borderWidth: 1, borderColor: 'rgba(74,222,128,0.3)', borderRadius: 10, padding: 12, marginBottom: 16 }}>
            <Text style={{ color: COLORS.success, fontSize: 13 }}>{t('settings.pwUpdated')}</Text>
          </View>
        ) : null}
        ...
        {success && (
          <Pressable onPress={() => router.back()} style={{ marginTop: 14, alignItems: 'center' }}>
            <Text style={{ color: COLORS.brand, fontWeight: '700' }}>{t('settings.backToSettings')}</Text>
          </Pressable>
        )}
      </ScrollView>
